...
describe("install command is registered as a top-level subcommand", () => {
	...
	test("CLI runner rejects only bare reserved management words", () => {
		expect(resolveCliArgv(["extensions"])).toEqual({
			error: '`clyean extensions` is not a management command. Use `clyean plugin list` / `clyean plugin install`, or run `clyean launch extensions` if you meant to send "extensions" as a prompt.',
		});
