...
pub const ORCHESTRATION_BRIDGE: ManagedExtension = ManagedExtension {
...
pub fn managed_extensions(agent: AgentId) -> Vec<ManagedExtension> {
    match agent {
        AgentId::UserAssistant => vec![HERDR_REPORTER, ORCHESTRATION_BRIDGE],
        _ => Vec::new(),
    }
}
...
mod tests {
    ...
    fn embedded_extensions_carry_version_markers() {
        ...
        assert_eq!(ORCHESTRATION_BRIDGE.version(), Some(1));
